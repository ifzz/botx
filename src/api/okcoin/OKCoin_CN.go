...
func (ctx *OKCoinCN_API) placeOrder(side, amount, price string, currency CurrencyPair) (*Order, error) {
	...
	order := new(Order)
	order.OrderID = int64(respMap["order_id"].(float64))
	order.OrderID2 = Sprintf("%f", respMap["order_id"].(float64))
...
func (ctx *OKCoinCN_API) getOrders(orderId string, currency CurrencyPair) ([]Order, error) {
	...
	for _, v := range orders {
		...
		var order Order
		order.Currency = currency
		order.OrderID = int64(orderMap["order_id"].(float64))
		order.OrderID2 = Sprintf("%f", orderMap["order_id"].(float64))
